...
const Accordion_component = ({ accordion_list }) => {
    ...
    return (
        <section className='pt-[80px] lg:pb-[80px] pb-[40px] px-4'>
            <div className='main_container'>
               <div className='lg:mb-20 mb-8'>
                  <h2 className='text_gradient inter lg:text-[36px] text-[18px] font-bold lg:leading-[52px] leading-[22px] mb-4 text-center'>Frequently Asked Question</h2>
               ...
               <div className='main_accordions'>
                {
                    accordion_list.map((list, index) => <div key={index} className='accordions mb-6'>
                        <div onClick={() => handleAccordion(index)} className={`py-[16px] px-[24px] flex justify-between items-center ${open === index ? "accordion_head active" : "accordion_head" }`}>
                            <h3 className='lg:text-[16px] text-[14px] text-[#333] w-[90%] '>{list.headContent}</h3>
                        ...
                        <div className={`${ open === index ? "accordion_paragraph active" : "accordion_paragraph"}`}>
                            <p className='lg:text-[16px] text-[14px] text-[#333] leading-7 font-light'>{list.paraContent}</p>
                        </div>
                    </div> 
                    )}
               </div>
